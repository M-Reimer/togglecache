...
async function ToolbarButtonClicked() {
  let value = (await browser.browserSettings.cacheEnabled.get({})).value;
  await browser.browserSettings.cacheEnabled.set({value: !value});
  await UpdateBadge();
}

// Sets browserAction badge text based on cache status.
async function UpdateBadge() {
  const value = (await browser.browserSettings.cacheEnabled.get({})).value;
...
IconUpdater.Init("icons/togglecache.svg");
